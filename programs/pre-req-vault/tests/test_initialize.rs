use {
    ...
    litesvm::LiteSVM,
    pre_req_vault::{registration, APPLICATION_ACCOUNT_SEED, VAULT_SEED, VAULT_STATE_SEED},
...
fn setup() -> (LiteSVM, Keypair) {
    ...
    let pre_req_vault_program_id = pre_req_vault::id();
    let pre_req_vault_bytes = include_bytes!("../../../target/deploy/pre_req_vault.so");
    svm.add_program(pre_req_vault_program_id, pre_req_vault_bytes)
        .unwrap();

    let registration_program_id = registration::ID;
    let registration_bytes = include_bytes!("./fixtures/registration.so");
    svm.add_program(registration_program_id, registration_bytes)
        .unwrap();
...
fn test_initialize_deposit_withdraw_close() {
    ...
    let (vault_pda, vault_bump) = Pubkey::find_program_address(
        &[VAULT_SEED, vault_state_pda.as_ref()],
        &pre_req_vault::id(),
    );

    let (application_account_pda, _) = Pubkey::find_program_address(
        &[APPLICATION_ACCOUNT_SEED, user.as_ref()],
        &registration::ID,
    );
    ...
    let withdraw_ix = Instruction {
        ...
        accounts: pre_req_vault::accounts::Withdraw {
            ...
            vault_state: vault_state_pda,
            vault: vault_pda,
            application_account: application_account_pda,
            application_program: registration::ID,
            system_program: SYSTEM_PROGRAM_ID,
        }
        .to_account_metas(None),
